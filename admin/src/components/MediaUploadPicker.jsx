...
export const getRecommendedDimensions = (customSize, label = '', category = '', mediaType = '') => {
  if (customSize) return customSize
  const combined = (label + ' ' + category).toLowerCase()

  if (combined.includes('logo')) {
    return '400 × 120 px (Transparent PNG / SVG)'
  }
  if (combined.includes('poster') || combined.includes('fallback')) {
    if (combined.includes('reel') || combined.includes('vertical') || combined.includes('results')) {
      return '1080 × 1920 px (9:16 Portrait / Reel Poster)'
    }
    return '1920 × 1080 px (16:9 Landscape Poster)'
  }
  if (mediaType === 'video' || combined.includes('video') || combined.includes('mp4')) {
    if (combined.includes('reel') || combined.includes('short') || combined.includes('vertical') || combined.includes('results')) {
      return '1080 × 1920 px (9:16 Vertical Reel Video, MP4 < 25MB)'
    }
    return '1920 × 1080 px (16:9 Full HD Landscape Video, MP4 < 30MB)'
  }
  if (combined.includes('hero') || combined.includes('slide') || combined.includes('carousel') || combined.includes('banner')) {
    return '1920 × 1080 px (16:9 Full HD Landscape)'
  }
  if (combined.includes('concern')) {
    return '600 × 800 px (3:4 Portrait)'
  }
  if (combined.includes('before') || combined.includes('after')) {
    return '800 × 800 px (1:1 High-Res Square)'
  }
  if (combined.includes('instagram') || combined.includes('social') || combined.includes('community')) {
    return '1080 × 1080 px (1:1 Square Post)'
  }
  if (combined.includes('avatar') || combined.includes('customer') || combined.includes('reviewer') || combined.includes('user')) {
    return '200 × 200 px (1:1 Square Avatar)'
  }
  if (combined.includes('product') || combined.includes('gallery') || combined.includes('main image')) {
    return '1000 × 1000 px (1:1 High-Res Square)'
  }
  if (combined.includes('category') || combined.includes('collection')) {
    return '600 × 600 px (1:1 Square)'
  }
  if (combined.includes('brand') || combined.includes('science') || combined.includes('story') || combined.includes('ritual') || combined.includes('feature')) {
    return '1200 × 800 px (3:2 Landscape)'
  }
  return '1000 × 1000 px (1:1 Square)'
}
...
export default function MediaUploadPicker({
  ...
  const displayRecommendedSize = getRecommendedDimensions(recommendedSize, label, category, mediaType)
  ...
  return (
    <div className="space-y-1.5 w-full">
      <div className="flex flex-wrap items-center justify-between gap-1.5">
        ...
        {displayRecommendedSize && (
          <span className="inline-flex items-center gap-1 text-[10px] font-mono font-bold text-blue-700 bg-blue-50/90 border border-blue-200/90 px-2 py-0.5 rounded-md shadow-2xs">
            <span>📐 Size:</span>
            <span>{displayRecommendedSize}</span>
          </span>
        )}
      </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col">
            ...
            {displayRecommendedSize && (
              <div className="bg-blue-50 border-b border-blue-100 px-5 py-2.5 flex items-center gap-2 text-xs text-blue-900 font-medium">
                <span className="p-1 rounded bg-blue-200/80 text-blue-800 font-bold text-[10px]">📐 RECOMMENDED</span>
                <span className="font-bold">{displayRecommendedSize}</span>
              </div>
            )}
