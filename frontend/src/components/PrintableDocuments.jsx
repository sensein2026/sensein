...
/**
 * 2. Official Ekart Logistics 4×6 Thermal Shipping Label (spec.yaml)
 */
export function EkartShippingLabel({ order }) {
  if (!order) return null
  const awb = order.ekart?.awbNumber || order.trackingNumber || order.delhivery?.waybill || '3148920184920'
  const isDelhivery = order.courierPartner?.includes('Delhivery') || (order.trackingNumber && !String(order.trackingNumber).startsWith('FMPC') && !String(order.trackingNumber).startsWith('EKT'))
  const sortCode = isDelhivery
    ? (order.delhivery?.sortCode || 'SRT/GCI')
    : (order.shippingAddress?.postalCode?.startsWith('395') ? 'SUR/EKT/395010' : 'AMD/EKT/380015')
  ...
  return (
    <div className="border-2 border-black p-4 bg-white text-black font-sans w-[380px] mx-auto text-xs leading-tight print:p-2 print:border-black print:text-black shadow-sm print:w-full print:max-w-[4in]">
      {/* Top Header: Delhivery / Ekart Logo + Courier Details */}
          ...
          {isDelhivery ? (
            <>
              ...
              <div className="text-[10px] font-bold leading-none">
                <div>EXPRESS B2C</div>
                <div className="text-[8px] text-neutral-600">DELHIVERY ONE</div>
              </div>
            </>
          ) : (
            <>
              <span className="bg-[#0074e4] text-white font-black text-sm px-2 py-0.5 tracking-tighter rounded">EKART</span>
              <div className="text-[10px] font-bold leading-none">
                <div>EXPRESS</div>
                <div className="text-[8px] text-neutral-600">FLIPKART GROUP</div>
              </div>
            </>
          )}
        </div>
      ...
      {/* Barcode representation for Courier AWB */}
      <div className="text-center py-2 border-b-2 border-black my-1">
        <div className="font-mono text-lg font-black tracking-widest leading-none my-1">{awb}</div>
        ...
        <div className="text-[9px] font-bold tracking-wider text-neutral-600">AWB TRACKING NUMBER ({isDelhivery ? 'DELHIVERY ONE' : 'EKART EXPRESS'})</div>
      </div>
      ...
      <div className="grid grid-cols-2 border-b-2 border-black py-1.5 text-center font-bold text-[11px]">
        <div className="border-r-2 border-black pr-1">
          <div className="text-[8px] text-neutral-600">ROUTING CODE</div>
          <div className="text-sm font-black">{sortCode}</div>
        </div>
        <div className="pl-1">
          <div className="text-[8px] text-neutral-600">PINCODE</div>
      ...
      <div className="py-2 border-b-2 border-black text-[10px]">
        <div className="font-bold text-neutral-600 uppercase">RETURN TO (SELLER HUB):</div>
        <div className="font-bold">Sensein Professional / MindNext</div>
      ...
      <div className="pt-2 flex justify-between items-center text-[10px]">
        ...
        <div className="text-right text-[9px] text-neutral-600">
          <div>Verified via {isDelhivery ? 'Delhivery B2C Engine' : 'Ekart Elite Engine'}</div>
          <div className="font-bold text-black">{isDelhivery ? 'https://one.delhivery.com' : 'https://app.elite.ekartlogistics.in'}</div>
        </div>
      </div>
    </div>
  )
...
/**
 * 3. Official Ekart Courier Handover Manifest (spec.yaml)
 */
export function EkartManifestSlip({ order, orders = [] }) {
  const manifestOrders = orders.length > 0 ? orders : (order ? [order] : [])
  const manifestRef = `EKT-MAN-${Date.now().toString().slice(-8)}`
    ...
    <div className="border-2 border-black p-6 bg-white text-black font-sans max-w-3xl mx-auto text-xs leading-tight print:p-2 print:border-black print:text-black shadow-sm">
      ...
      <div className="flex justify-between items-start border-b-2 border-black pb-3 mb-3">
        <div className="flex items-center gap-2">
          <span className="bg-[#0074e4] text-white font-black text-base px-2 py-0.5 rounded">EKART</span>
          <div>
            <div className="text-sm font-black uppercase">Courier Pickup Manifest Slip</div>
            <div className="text-[10px] text-neutral-600">
              Official B2C Package Handover Sheet for Ekart Logistics Executive
            </div>
          </div>
        </div>
        <div className="text-right text-[11px]">
          ...
          <div><span className="font-bold">Origin Hub:</span> Surat Central (395010)</div>
        </div>
      </div>
        ...
        <table className="w-full text-left text-[11px] border-collapse">
          <thead>
            <tr className="border-b-2 border-black bg-neutral-100 font-bold uppercase text-[10px]">
              ...
              <th className="p-2 border-r border-black">Ekart AWB</th>
              <th className="p-2 border-r border-black">Destination Hub</th>
          ...
          <tbody>
            {manifestOrders.map((o, idx) => {
              const awb = o.ekart?.awbNumber || o.trackingNumber || o.delhivery?.waybill || 'FMPC9688594552'
              const sortCode = o.shippingAddress?.postalCode?.startsWith('395') ? 'SUR/EKT/395010' : 'AMD/EKT/380015'
              ...
              return (
                <tr key={idx} className="border-b border-black">
                  ...
                  <td className="p-2 border-r border-black font-mono font-bold">{awb}</td>
      ...
      <div className="grid grid-cols-2 gap-6 border-t-2 border-black pt-4 mt-6 text-[11px]">
        <div>
          ...
          <div>Carrier Partner: <span className="font-bold">Ekart Logistics Express Network</span></div>
        </div>
        <div className="space-y-6 text-right">
          <div className="border-t border-black pt-1 font-bold">Ekart Pickup Executive Signature & FE ID</div>
...
export const DelhiveryShippingLabel = EkartShippingLabel
export const DelhiveryManifestSlip = EkartManifestSlip
