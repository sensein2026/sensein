...
import { AlertTriangle, X, CheckCircle2, Loader2, ShieldAlert, Sparkles, RefreshCw } from 'lucide-react'
import {
...
export default function MaintenanceControlModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        addToast({
          type: formData.isMaintenanceMode ? 'warning' : 'success',
          title: formData.isMaintenanceMode ? 'Maintenance Mode ON' : 'Website Live',
          message: res.message || 'Site settings updated successfully',
        })
        refetch()
        onClose()
      }
      ...
      addToast({
        type: 'error',
        title: 'Update Failed',
        message: err?.data?.message || err.message || 'Failed to update maintenance settings',
      })
    }
  }
